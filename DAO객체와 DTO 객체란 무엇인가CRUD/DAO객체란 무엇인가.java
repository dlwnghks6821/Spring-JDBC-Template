...
public class BDao {
	...
	//==> DAO <==//
	//DAO (Data Access Object) ==> DB(Data Base Access)//
	//DAO ==> DB 
	// 예시1) JSP(view) ==> DATA ==> Controller ==> DAO //
	// 예시2) Controller ==> DATA ==> DAO ==> DTO ==> view //
	// 예시3) Controller ==> DATA ==> DAO ==> Controller ==> Model ==> view //
	//--> 결론적으로 DAO ==> 데이터에 접근하는 객체 <--
	//==> DAO ==> DATA ACCESS ==> DATA ACEES == DB ( DATA BASE ) ==> Create , Read , Update , Delete
		...
		this.template.update(query, new PreparedStatementSetter() {
			...
			public void setValues(PreparedStatement ps)throws SQLException{
				ps.setInt(1, bId);
						...
						//Delete(선택한 게시물의 내용을 삭제하는 Delete)//
